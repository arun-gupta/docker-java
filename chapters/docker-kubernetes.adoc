...
. *Node* (née minions) is a worker node that run tasks as delegated by the master. Nodes can run one or more pods. It provides an application-specific "`virtual host`" in a containerized environment.
...
[[Access_Kubernetes_Application_Minion]]
...
----
+
. Access the application using <<Access_Kubernetes_Application_Minion>> or <<Access_Kubernetes_Application_Proxy>>.
