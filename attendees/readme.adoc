...
. Download Apache Maven as:
  ...
  curl http://dockerlab:8082/downloads/apache-maven-3.3.3-bin.zip
...
Download `lab-settings.xml` file from http://dockerlab:8082/downloads/lab-settings.xml

Edit the model file and replace the `dockerlab` with the accurate value.
...
## Configure Lab Docker Host

Add a host entry for the Docker Host running on your machine. To make it easier to access the containers, we add an entry into the host mapping table of your operating system.
