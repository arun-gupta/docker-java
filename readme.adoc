...
So, in order to access to WildFly server, we need to know which address has been chosen by the application server for socket binding. We will use the `docker inspect` command to find the assigned IP address:
...
The argument is the container id obtained earlier.

The above command outputs an IP address, such as ``172.17.0.1''. That means, the server is available at 172.17.0.1.
...
But ``docker0'' is no ordinary interface. It is a virtual Ethernet bridge that automatically forwards packets between any other network interfaces that are attached to it. This lets containers communicate both with the host machine and with each other, and nothing more.
...
### Enabling WildFly Admin Console (TODO)
